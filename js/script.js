const translations = {
    'en': {
        loading: 'Loading...', ipSearchPlaceholder: 'Enter IP address to search', copyright: 'All rights reserved.',
    ...
    'fa': {
        ...
        enabled: 'فعال', disabled: 'غیرفعال', notSpecified: 'مشخص نشده', supported: 'پشتیبانی می‌شود', notSupported: 'پشتیبانی نمی‌شود', available: 'در دسترس', notAvailable: 'در دسترس نیست', secure: 'امن', insecure: 'نا امن', online: 'آنلاین', offline: 'آفلاین', darkMode: 'حالت تاریک', lightMode: 'حالت روشن', systemPreference: 'تنظیمات سیستم', notDetected: 'شناسایی نشد / ناشناخته', unknown: 'ناشناخته', unavailable: 'در دسترس نیست', invalidIP: 'آی‌پی نامعتبر'
    },
    'de': {
        loading: 'Wird geladen...', ipSearchPlaceholder: 'IP Adresse zur Suche eingeben', copyright: 'Alle Rechte vorbehalten.',
    ...
    'es': {
        loading: 'Cargando...', ipSearchPlaceholder: 'Ingrese la dirección IP para buscar', copyright: 'Todos los derechos reservados.',
    ...
    'ru': {
        ...
        enabled: 'Включено', disabled: 'Отключено', notSpecified: 'Не указано', supported: 'Поддерживается', notSupported: 'Не поддерживается', available: 'Доступно', notAvailable: 'Недоступно', secure: 'Безопасно', insecure: 'Небезопасно', online: 'В сети', offline: 'Не в сети', darkMode: 'Темный режим', lightMode: 'Светлый режим', systemPreference: 'Системные настройки', notDetected: 'Не определено / Неизвестно', unknown: 'Неизвестно', unavailable: 'Недоступно', invalidIP: 'Неверный IP'
    },
    'ja': {
        loading: '読み込み中...', ipSearchPlaceholder: '検索するIPアドレスを入力', copyright: 'All rights reserved.',
        ...
        enabled: '有効', disabled: '無効', notSpecified: '指定なし', supported: '対応', notSupported: '非対応', available: '利用可能', notAvailable: '利用不可', secure: '安全', insecure: '危険', online: 'オンライン', offline: 'オフライン', darkMode: 'ダークモード', lightMode: 'ライトモード', systemPreference: 'システム設定', notDetected: '未検出/不明', unknown: '不明', unavailable: '利用不可', invalidIP: '無効なIP'
    },
    'ko': {
        loading: '로드 중...', ipSearchPlaceholder: '검색할 IP 주소 입력', copyright: 'All rights reserved.',
    ...
    'zh': {
        loading: '加载中...', ipSearchPlaceholder: '输入IP地址进行搜索', copyright: '版权所有。',
        ...
        enabled: '已启用', disabled: '已禁用', notSpecified: '未指定', supported: '支持', notSupported: '不支持', available: '可用', notAvailable: '不可用', secure: '安全', insecure: '不安全', online: '在线', offline: '离线', darkMode: '深色模式', lightMode: '浅色模式', systemPreference: '系统偏好', notDetected: '未检测到/未知', unknown: '未知', unavailable: '不可用', invalidIP: '无效IP'
    }
};
...
async function fetchIPInfo(query = '') {
    ...
    let currentIpv4 = translationSet.unavailable;
    let currentIpv6 = translationSet.unavailable;
    let targetIP = query.trim();

    if (targetIP === '') {
        try {
            const ipv4Response = await fetch('https://api.ipify.org?format=json');
            if (ipv4Response.ok) currentIpv4 = (await ipv4Response.json()).ip;
            targetIP = currentIpv4;
            const ipv6Response = await fetch('https://api6.ipify.org?format=json');
            if (ipv6Response.ok) currentIpv6 = (await ipv6Response.json()).ip;
            elements.ipv4Item.style.display = 'flex';
            if (currentIpv6 !== translationSet.unavailable) {
                elements.ipv6Item.style.display = 'flex';
            } else {
                elements.ipv6Item.style.display = 'none';
            }
        } catch (e) {
            setTextContent(elements.ipAddress, translationSet.unavailable);
            setTextContent(elements.ipv6Address, translationSet.unavailable);
            hidePreloader();
            setTimeout(() => {
                elements.refreshNetworkButton.disabled = false;
            }, REFRESH_COOLDOWN);
    ...
    } else if (!isValidIP(targetIP)) {
        setTextContent(elements.ipAddress, '', 'invalidIP');
        setTextContent(elements.ipv6Address, 'N/A');
        hidePreloader();
        setTimeout(() => {
            elements.refreshNetworkButton.disabled = false;
        }, REFRESH_COOLDOWN);
        ...
        currentIpv4 = targetIP;
        elements.ipv4Item.style.display = 'flex';
        elements.ipv6Item.style.display = 'none';
    }

    if (targetIP !== translationSet.unavailable && targetIP !== '') {
        try {
            const geoResponse = await fetch(`https://api.findip.net/${targetIP}/?token=${FINDIP_TOKEN}`);
            if (geoResponse.ok) {
                lastGeoData = await geoResponse.json();
                displayGeoData(lastGeoData);
            } else {
                lastGeoData = null;
            ...
            lastGeoData = null;
            setTextContent(elements.country, translationSet.unavailable);
            setTextContent(elements.city, translationSet.unavailable);
            setTextContent(elements.isp, translationSet.unavailable);
            setTextContent(elements.organization, translationSet.unavailable);
            setTextContent(elements.connectionType, translationSet.unavailable);
            setTextContent(elements.ipTimezone, translationSet.unavailable);
            if (elements.regionItem) elements.regionItem.style.display = 'none';
        }
    } else {
        lastGeoData = null;
    ...
    setTextContent(elements.ipAddress, currentIpv4);
    setTextContent(elements.ipv6Address, currentIpv6);

    const fields = [
        { element: elements.ipAddress, id: 'ip-address', invalidKeys: ['unknown', 'unavailable', 'invalidIP'] },
        { element: elements.ipv6Address, id: 'ipv6-address', invalidKeys: ['unknown', 'unavailable'] },
        { element: elements.connectionType, id: 'asn', invalidKeys: ['unknown', 'unavailable'] },
        { element: elements.isp, id: 'isp', invalidKeys: ['unknown', 'unavailable'] },
        { element: elements.gpu, id: 'gpu', invalidKeys: ['unknown', 'unavailable', 'notDetected'] }
    ];

    fields.forEach(field => {
        const translationSet = translations[currentLang] || translations['en'];
        const invalidValues = field.invalidKeys.map(key => translationSet[key]);
        if (!invalidValues.includes(field.element.textContent)) {
            addCopyFeature(field.id);
        }
    });
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    elements.refreshNetworkButton?.addEventListener('click', () => fetchIPInfo(elements.ipDomainSearch.value));
    elements.ipDomainSearch.addEventListener('keypress', (event) => { if (event.key === 'Enter') fetchIPInfo(elements.ipDomainSearch.value); });
    elements.searchButton?.addEventListener('click', () => fetchIPInfo(elements.ipDomainSearch.value));
